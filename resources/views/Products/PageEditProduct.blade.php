...
<div class="container mt-5">
    <div class="p-4 border-0 shadow-sm card rounded-3">
        <h3 class="mb-4 text-center fw-bold text-primary"> Edit {{$product->name ?? "invalid"}}</h3>
        @if (session()->has('success'))
        <h3 class="mb-4 text-center fw-bold text-primary"> {{session('success')}}</h3>            
        @endif
        <form action="EditCategoryData-{{$product->id}}" method="POST" enctype="multipart/form-data">
            @csrf
            <!-- Product Info -->
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control custom-input" value="{{$product->name ?? "invalid"}}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">SKU</label>
                    <input type="text" name="sku" class="form-control custom-input" value="{{$product->sku ?? "invalid"}}">
                </div>
            </div>
            <br>
            <!-- Pricing -->
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="number" name="price" step="0.01" class="form-control custom-input" value="{{$product->price ?? "invalid"}}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Discount Price ($)</label>
                    <input type="number" name="discount_price" step="0.01" class="form-control custom-input" value="{{$product->discount_price ?? "invalid"}}">
                </div>
            </div>
            <br>
            <!-- Category & Status -->
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="number" name="stock" class="form-control custom-input" value="{{$product->stock ?? "invalid"}}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Expiry Date</label>
                    <input type="date" name="expire_date" class="form-control custom-input" value="{{$product->expire_date ?? "invalid"}}">
                </div>
            </div>
            <br>
            <!-- Extra Info -->
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="number" name="weight" step="0.01" class="form-control custom-input" value="{{$product->weight ?? "invalid"}}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Dimensions (LxWxH)</label>
                    <input type="text" name="dimensions" class="form-control custom-input" value="{{$product->dimensions ?? "invalid"}}">
                </div>
            </div>
            <br>
            <!-- Image Upload -->
            ...
            <div class="mt-3">
                ...
                <textarea class="form-control custom-input" name="description" rows="3" >{{$product->description}}</textarea>
            </div>
            <br>
            <!-- Department & Featured -->
            <div class="mt-1 row g-3">
